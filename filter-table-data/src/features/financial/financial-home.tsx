...
class FinancialHome extends React.Component<any, IFinancialDetail> {
    ...
    changeFinancialData(event: any) {
        const changedData = this.createDataObj(event.target.name, event.target.value);
    ...
    changeNonFinancialData(event: any) {
        const changedData = this.createDataObj(event.target.name, event.target.value);
        let dataObj: IFormProps = Object.assign({}, this.state.nonFnPreviousDetails, changedData);
        this.setState({ nonFnPreviousDetails: dataObj});
        event.preventDefault();
    }
    changeOthersData(event: any) {
        const changedData = this.createDataObj(event.target.name, event.target.value);
        let dataObj: IFormProps = Object.assign({}, this.state.othPreviousDetails, changedData);
        this.setState({ othPreviousDetails: dataObj});
        event.preventDefault();
    }
    getFinancialOldData() {
        this.setState({ fnNewDetails: this.state.fnPreviousDetails});
    }
